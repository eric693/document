"""
blueprints/shifts.py — 班別 CRUD、班表指派、匯入匯出、衝突偵測
"""
...
from blueprints.punch import shift_type_row, shift_assign_row
    ...
    with get_db() as conn:
        ...
        if not force:
            leave_rows = conn.execute("""
                SELECT staff_id, dates FROM schedule_requests WHERE status='approved'
            """).fetchall()
...
def api_shift_conflicts():
    ...
    for r in rows:
        ...
        if cross:
            conflicts.append({
                'type': 'midnight_cross', 'severity': 'info',
                'date': str(r['shift_date']), 'staff_name': r['staff_name'],
        ...
        if hrs > 10:
            conflicts.append({
                'type': 'overtime_hours', 'severity': 'warning' if hrs <= 12 else 'error',
                'date': str(r['shift_date']), 'staff_name': r['staff_name'],
                'shift_name': r['shift_name'],
        ...
        for i in range(1, len(dates)):
            if (dates[i] - dates[i-1]).days == 1:
                ...
                if len(streak) >= 6:
                    ...
                    conflicts.append({
                        'type': 'consecutive_days', 'severity': sev,
                        'date': streak[0].isoformat(), 'staff_name': info['name'],
                        'shift_name': '',
                        'message': (f"連續排班 {len(streak)} 天"
...
def api_shift_export():
    ...
    for d in range(1, days_in_month + 1):
        ...
        c.fill = (PatternFill('solid', fgColor='1A3060') if not (is_wkend or is_hol)
                  else PatternFill('solid', fgColor='8B2020'))
    ...
    for row_idx, staff in enumerate(staff_list, start=2):
        ...
        for d in range(1, days_in_month + 1):
            ...
            c.alignment = center; c.border = thin_border
            if not val:
                c.fill = (wkend_fill if wd >= 5 else (hol_fill if ds in holidays
...
@require_module('sched')
def api_staffing_requirements_get():
    with get_db() as conn:
        rows = conn.execute("""
            SELECT ssr.*, st.name as shift_name
            FROM shift_staffing_requirements ssr
            JOIN shift_types st ON st.id=ssr.shift_type_id
            ORDER BY ssr.shift_type_id, ssr.day_of_week
        """).fetchall()
...
@require_module('sched')
def api_staffing_requirements_put():
    items = request.get_json(force=True) or []
    with get_db() as conn:
        for item in items:
            conn.execute("""
                INSERT INTO shift_staffing_requirements (shift_type_id, day_of_week, min_staff)
                VALUES (%s,%s,%s)
                ON CONFLICT (shift_type_id, day_of_week) DO UPDATE SET min_staff=%s
            """, (item['shift_type_id'], item['day_of_week'], item['min_staff'], item['min_staff']))
