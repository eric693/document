"""
blueprints/finance.py — 財務模組（收支記錄、財務報表、保險設定、EDI 匯出）
"""
...
def api_bank_import():
    """匯入銀行對帳單 CSV"""
    import csv
    from datetime import datetime as _dt_bank
    file = request.files.get('file')
    ...
    import_batch = _dt_bank.now(TW_TZ).strftime('%Y%m%d%H%M%S')
    ...
    with get_db() as conn:
        for r in rows_data:
            if len(r) < 2: continue
            date_str = _parse_date(r[0])
            if not date_str: continue
            desc = r[1].strip() if len(r) > 1 else ''
            if len(r) >= 4:
                debit  = _parse_amount(r[2])
                credit = _parse_amount(r[3])
...
def api_bank_summary():
    ...
    for k in d:
        if d[k] is not None:
            try:
                d[k] = float(d[k]) if '.' in str(d[k]) else int(d[k])
...
@require_module('finance')
def api_insurance_settings_save():
    data = request.get_json(force=True)
    with get_db() as conn:
        for k, v in data.items():
            conn.execute(
                "INSERT INTO insurance_settings (setting_key, setting_value) VALUES (%s,%s) ON CONFLICT (setting_key) DO UPDATE SET setting_value=EXCLUDED.setting_value",
                (k, str(v))
            )
    return jsonify({'ok': True})
